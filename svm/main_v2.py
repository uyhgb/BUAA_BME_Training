...
LABELED_IMU_PATH = os.path.join(BASE_DIR, "labeled_imu_data_20251220_212156.csv")
...
if __name__ == "__main__":
    ...
    # except Exception as e:
    #     error_msg = str(e).encode('ascii', errors='ignore').decode('ascii')
    #     print(f"SVM training failed: {error_msg}, switching to Random Forest...")
    #     model, metrics = train_fallback_model(X_scaled, y, y_phase)
 
 
    # 修改 if __name__ == "__main__": 下面的训练部分

    # --- 修改开始 ---
    # 暂时注释掉 try-except，强制运行 SVM
    # try:
    print("正在强制训练 SVM 模型...")
    model, metrics = train_optimized_model(X_scaled, y, y_phase)

    # 如果 SVM 效果不好再切换（这是原本的逻辑，现在先不管）
    if metrics['R²决定系数'] < 0.6:
        print("SVM 效果不佳 (R² < 0.6)，切换到随机森林...")
        model, metrics = train_fallback_model(X_scaled, y, y_phase)
